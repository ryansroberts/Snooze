...
{
    internal class SubResourceRoute<T, P> : ResourceRoute<T>
        where T : SubUrl<P>
        where P : Url
    {
        ...
        static Expression<Func<T, string>> ConcatUriExpressions(Expression<Func<P, string>> parent,
                                                                Expression<Func<T, string>> child)
        {
