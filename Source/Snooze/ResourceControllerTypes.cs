...
namespace Snooze
{
    internal static class ResourceControllerTypes
    {
        static readonly Dictionary<Type, Type> _controllerTypes = new Dictionary<Type, Type>();
            ...
            else
            {
                lock (_controllerTypes)
                {
                    ...
                    var oneControllerPerUrlType =
                        ...
                        group t by t.urlType
                        into urlType
                        where !urlType.Key.IsAbstract
                        select new {urlType = urlType.Key, urlType.First().controllerType};
